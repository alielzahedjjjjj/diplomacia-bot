...
def refresh_profile(acc_id):
    ...
    try:
        ...
        acc['xp_pct'] = round(lp.get('percentage', 0) * 100) if lp.get('percentage', 0) <= 1 else round(lp.get('percentage', 0))
        acc['level_num'] = player.get('level', '?')

        # skills هي أرقام مباشرة: {"kisla": 68, "savas_teknikleri": 67, "bilim_insani": 115}
...
def get_cooldown(acc_id):
    ...
    if data:
        try:
            ...
            # لو مفيش cooldown data — جاهز للترقية
            upgrading = data.get('is_upgrading') or data.get('upgrading', False)
            if upgrading:
                return 60  # افتراضي
            return 0
        except Exception as e:
    ...
    try:
        ...
        # لو skills أرقام بسيطة — مفيش cooldown info فيها، افترض جاهز
...
def do_upgrade(acc_id):
    ...
    # endpoints مرتبة من الأكثر احتمالاً للصح
    endpoints_payloads = [
